...
const MainLayout: React.FC = () => {
    ...
    // Determine dashboard link based on role, default to hunter if no profile yet
    const dashboardLink = profile?.role === 'payer' ? '/payer/dashboard' : '/hunter/dashboard';

    // Check if we are on auth pages to potentially simplify header
    ...
    return (
        <div className="min-h-screen bg-iq-black text-iq-text font-sans selection:bg-iq-green selection:text-iq-black flex flex-col">
                ...
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        {user ? (
                            <Link
                                to={dashboardLink}
                                className="px-4 py-2 bg-iq-green text-iq-black font-bold text-sm rounded hover:shadow-[0_0_15px_rgba(0,255,157,0.4)] transition-all flex items-center gap-2"
                            >
                            ...
                            <>
                                {!isAuthPage && (
                                    <Link to="/signin" className="text-sm font-medium hover:text-iq-green transition-colors text-white">Sign In</Link>
                                )}
                                <Link
            ...
            {/* Hide footer on auth pages to reduce distraction */}
            {!isAuthPage && (
                    ...
                    <div className="max-w-7xl mx-auto px-6 py-12 grid md:grid-cols-4 gap-8">
                        <div className="space-y-4">
                            <Link to="/" className="flex items-center gap-2">
                                ...
                                <span className="font-display font-bold text-lg text-white">IQHUNT</span>
                            </Link>
